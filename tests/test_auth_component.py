...
class AuthTestSuite(unittest.TestCase):
    ...
    def test_signup_success(self):
        ...
        code = self._get_verification_code('user@example.com')
        self.assertTrue(code)

    def test_signup_duplicate_email(self):
        self._signup()
        resp = self._signup()
        self.assertIn(b'Email already exists.', resp.data)

    def test_signin_blocked_until_verified(self):
        self._signup()
        resp = self.client.post('/signin', data={
            'email': 'user@example.com', 'password': 'Password123!'
        }, follow_redirects=True)
        self.assertIn(b'Please verify your email before signing in.', resp.data)

    def test_email_verification_and_signin(self):
        self._signup()
        code = self._get_verification_code('user@example.com')
        self.assertTrue(code)
        verify_resp = self.client.get(f'/signup/{code}', follow_redirects=True)
        self.assertIn(b'Email verified successfully', verify_resp.data)
        signin_resp = self.client.post('/signin', data={
            'email': 'user@example.com', 'password': 'Password123!'
        }, follow_redirects=True)
        self.assertIn(b'Signed in successfully', signin_resp.data)
        with self.client.session_transaction() as sess:
            self.assertIn('customer_id', sess)

    def test_invalid_verification_token(self):
        resp = self.client.get('/signup/invalidtoken', follow_redirects=True)
        self.assertIn(b'Invalid or expired verification link.', resp.data)

    def test_logout_clears_session(self):
        self._signup()
        code = self._get_verification_code('user@example.com')
        self.client.get(f'/signup/{code}', follow_redirects=True)
        self.client.post('/signin', data={'email': 'user@example.com', 'password': 'Password123!'})
        with self.client.session_transaction() as sess:
            self.assertIn('customer_id', sess)
        out_resp = self.client.get('/logout', follow_redirects=True)
        self.assertIn(b'You have been logged out.', out_resp.data)
        with self.client.session_transaction() as sess:
            self.assertNotIn('customer_id', sess)

    def test_test_endpoint_forbidden_wrong_key(self):
        self._signup()
        r = self.client.post('/_test/get_verification_token', json={
            'email': 'user@example.com', 'key': 'WRONG'
        })
        self.assertEqual(r.status_code, 403)
        self.assertIn(b'forbidden', r.data)

    def test_test_endpoint_returns_token_then_null_after_verification(self):
        self._signup()
        r = self.client.post('/_test/get_verification_token', json={
            'email': 'user@example.com', 'key': 'TEST_KEY_123'
        })
        token = r.json['token']
        self.assertTrue(token)
        self.client.get(f'/signup/{token}', follow_redirects=True)
        r2 = self.client.post('/_test/get_verification_token', json={
            'email': 'user@example.com', 'key': 'TEST_KEY_123'
        })
